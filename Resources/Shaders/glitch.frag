...
void main()
{
	...
	rowGlitch *= step(fract(time*1.3-0.01+1.3*noiseCol.y), gl_TexCoord[0].y) * (1-step(fract(time*1.3+0.01+1.3*noiseCol.y), gl_TexCoord[0].y))
	...
	float red = texture2D(diffuseTex, gl_TexCoord[0].xy+noiseCol.yx*(noisestrength+rowGlitch) + vec2(fract(time),0)*noisestrength).r;
	float green = texture2D(diffuseTex, gl_TexCoord[0].xy+noiseCol.yx*(noisestrength+rowGlitch) + vec2(0,fract(time))*noisestrength).g;
