...
void sd::ShaderEngine::setup_all_shader() {
    ...
    shader_procedures_.emplace_back(std::make_shared<GlowingPulse>(Sp<sf::Shader>(glowing_pulse_)));
}
...
void sd::ShaderEngine::set_glowing_pulse_on(std::string object_name) const {
    for (const auto& object : drawable_objects_) {
        if (object->get_name () == object_name) {
            object->set_shader_procedure (shader_procedures_[5]);
        }
    }
}
